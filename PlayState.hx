...
importar flixel.input.keyboard.FlxKey;
importar openfl.events.KeyboardEvent;
Importar Logros;
import StageData;
importar FunkinLua;
...
clase PlayState extiende MusicBeatState
{
	...
	public var songSpeed ​​(predeterminado, establecido): Float = 1;
	...
	var sicks públicos: Int = 0;
	bienes públicos var: Int = 0;
	varones de var público: Int = 0;
	var pública mierda: Int = 0;
	
	private var generateMusic: Bool = falso;
	...
	practMode var public: Bool = falso;
	...
	// Lua mierda
	instancia de var estática pública: PlayState;
	...
	// Controles menos retrasados
	private var keysArray: Array <dinámico>;
	...
	anular la función pública create ()
	{
		...
		// Por el logro "Solo nosotros dos"
		para (i en 0 ... keysArray.length)
		{
			keysPressed.push (falso);
		}
		...
		healthLoss = ClientPrefs.getGameplaySetting ('pérdida de salud', 1);
		instakillOnMiss = ClientPrefs.getGameplaySetting ('instakill', falso);
		practiceMode = ClientPrefs.getGameplaySetting ('práctica', falso);
		cpuControlled = ClientPrefs.getGameplaySetting ('botplay', falso);
		...
		// Cadena que contiene el modo definido aquí, por lo que no es necesario llamar a change
		si (isStoryMode)
		{
		...
		var folderToCheck: Array <String> = [Main.getDataPath () + Paths.getPreloadPath ('scripts /')];

		#if MODS_ALLOWED
		folderToCheck.insert (0, Paths.mods ('scripts /'));
		if (Paths.currentModDirectory! = null && Paths.currentModDirectory.length> 0)
			folderToCheck.insert (0, Paths.mods (Paths.currentModDirectory + '/ scripts /'));
		#fin

		para (carpeta en folderToCheck)
		{
			si (FileSystem.exists (carpeta))
			{
				para (archivo en FileSystem.readDirectory (carpeta))
				{
					if (file.endsWith ('. lua') &&! filesPushed.contains (file))
					{
						luaArray.push (nuevo FunkinLua (carpeta + archivo));
						filesPushed.push (archivo);
					}
				}
			}
		}
		...
		if (FileSystem.exists (Paths.modFolders (luaFile))) {
			...
			si (FileSystem.exists (luaFile)) {
				doPush = verdadero;
			}
		}

		si (doPush) 
			luaArray.push (nuevo FunkinLua (luaFile));
		#fin
		...
		if (gfVersion == null || gfVersion.length <1) {
			...
			SONG.gfVersion = gfVersion; // Arreglo para el editor de gráficos
		}
